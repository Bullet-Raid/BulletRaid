...
public class PlayerBehavior : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKey(KeyCode.LeftShift)) {playerVelocity = 1;}
		float yPos = gameObject.transform.position.y + (Input.GetAxis ("Vertical")) * Time.deltaTime * playerVelocity;
		float xPos = gameObject.transform.position.x + (Input.GetAxis ("Horizontal")) * Time.deltaTime * playerVelocity;
		...
		for (int i = 0; i < Bullets.Count; i++) {
			...
			if (moveBullet != null) {
				moveBullet.transform.Translate(new Vector3(0, 1, 0) * Time.deltaTime * bulletVelocity);
